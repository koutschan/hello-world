...
solutions are $1$ and $\log(x)$.  We must admit that the class of functions
considered in this paper is not as large as it may seem at first glance, because
...
$h_i = u_i d + v_i e$ and $\deg_x(v_i) < \deg_x(d)$. Then we get
\[
  \sum_{i=1}^n \frac{h_i}{de}\omega_i =  \sum_{i=1}^n \left(\frac{u_i}{e} + \frac{v_i}{d}\right)\omega_i.
\]

We now introduce the \emph{polynomial reduction} for confining the $u_i$ to a finite-dimensional
...
be defined by $\phi_V(p) = a \partial_x(p) + pB$ for any $p\in K[x]^n$.
...
Note that, by construction and using Lemma~\ref{LEM:d}, $q\in K[x]^n$ is in
$\im(\phi_V)$ if and only if $(1/a)qV$ is integrable in~$A$.
\end{defi}
...
\[
  B = B_rx^r + \ldots + B_0,
\]
where $r=\deg_x(B)=\max_{i,j}\deg_x(b_{i,j})$ and $B_i\in K^{n\times n}$.
...
Let $N_V$ be the subspace of $K[x]^n$ spanned by
\[\{t \in \cS \mid t \neq \lt(p) \ \text{for all $p\in \im(\phi_V)$}\}.\]
Then $K[x]^n = \im(\phi_V) \oplus N_V$.
We call $N_V$ the \emph{standard complement} of $\im(\phi_V)$.


We now explain how to determine the dimension and a basis for $N_V$ when $\deg_x(B) \leq  \mu := \deg_x(a)-1$.
...
with $j\geq \mu +1$ and $i\in \{1, \ldots, n\}$ is not in $N_V$. Let $K[x]_\mu^n := \{p\in K[x]^n \mid \deg_x(p) \leq \mu\}$.
...
(in the following the decoration~\raisebox{-1pt}{$\vec{}\;$} always indicates such a typecast).
Similarly, the columns of the matrix $B\in K[x]^{n\times n}$ can be expressed as vectors $\vec{B}_1, \ldots, \vec{B}_n$.
Let $C(B)$ be the subspace of $K[x]_\mu^n$ generated by these column vectors over~$K$.
If $q\in \im(\phi_V)$, then $q = \phi_V(p) = pB$ for some $p \in K^n$, which implies
$\vec{q}$  is a linear combination of $\vec{B}_i$'s. Then $K[x]_\mu^n = C_B \oplus N_V$.
So $\dim_K(N_V)= (\mu+1)n - \dim(C(B))$ and a basis of $N_V$ can be computed by
...
the matrix $J_s = s\lc(a) + B_{\mu}$ is invertible. So any monomial $e_ix^j$ with $j> \lambda_0+\mu$ is not in~$N_V$
...
Then $q\in\im(\phi_v)$ if and only if $\vec{q}$ is in the column space of $M_{\lambda_0}$.
Therefore,
\[K[x]_{\lambda_0+\mu}^n = C({M_{\lambda_0}}) \oplus N_V. \]
This implies that $\dim_K(N_V) = n(\lambda_0+\mu+1) - \rank({M_{\lambda_0}})$, and
a basis of $N_V$ can be computed by
looking at the echelon form of the matrix ${M_{\lambda_0}}$.
As a consequence of the above discussions about the complement space $N_V$, we get the following result.
...
%%Thus, the dimension of $N_V$ is at most $n (\deg_x(a)-1)$.
%%
%%Next we consider the case when $\mu =\deg_x(a)-1$. Then
%%\[\phi_V(p) = (s\lc(a) + B_{\mu})\lc(p)x^{s+\mu}+\text{lower terms in~$x$}.\]
%%If $-s\lc(a)\in K$ is not an eigenvalue of $B_{\mu}\in K^{n\times n}$, then
%%the matrix $J_s = s\lc(a) + B_{\mu}$ is invertible and then $e_ix^{s+\mu}$ is not in $N_V$
%%for any $i=1, \ldots, n$. Since $B_\mu$ has at most $n$ eigenvalues, the dimension of $N_V$
...
and $U_2\in N_W$. Then we have
\[r = \partial_x(U_1W) + \frac{1}{e} U_2W. \]
Note that $U_2\in N_W$ belongs to a finite-dimensional vector space over~$K$.
When the inequality $\deg_x(M)\leq \deg_x(e)-1$ does hold
for the integral basis~$W$, we can
change the basis into a local integral basis at infinity. If $W$ is also normal at infinity,
...
$\tilde{U}_2\in N_V$. This means $r$ is decomposed into the sum of an integrable function
and a non-integrable one that lies in a finite-dimensional vector space over~$K$.

Combining the Hermite reduction and polynomial reduction, we get the following theorem.
\begin{theorem}\label{THM:polyred}
Let $W$ be an integral basis of~$A$ that is normal at infinity. Let $T := \diag(x^{-r_1}, \ldots, x^{-r_n}) \in K(x)^{n\times n}$
