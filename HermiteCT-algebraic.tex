...
\title{Reduction-Based Creative~Telescoping for \\ Algebraic Functions\titlenote{S.\ Chen was supported by the NSFC grant 11501552 and
...
based on Trager's Hermite reduction and polynomial reduction, which was originally designed for
...
$f=\frac1d(a_0+a_1y+\cdots a_{n-1}y^{n-1})$ for some $a_0,\dots,a_{n-1},d\in K[x]$
with $\gcd(a_0,\dots,a_{n-1},d)=1$. 
...
Since $\bar K\<1/x>$ and the $\bar K\<x-a>$ are fields, we can associate to every 
...
of $m$ into the representation $f=\frac1d(a_0+a_1y+\cdots+a_{n-1}y^{n-1})$. 
In other words, for every $a\in\bar K\cup\{\infty\}$ there are $n$ distinct natural
ring homomorphisms from $A$ to $K\<x-a>$ or $K\<1/x>$, respectively. 

In the fields $A$ as well as the fields $\bar K\<x-a>$ and $\bar K\<1/x>$, we have
...
Note also that integrality at $a\in\bar K$ is not preserved by differentiation,
...
called a \emph{local integral basis} at $a$ for~$A$. 
...
This reduction was later extended to liouvillean functions by Risch~\cite{..,..},
and to algebraic functions by Trager~\cite{trager84}. 
%% MK: AS FAR AS HERMITE REDUCTION IS CONCERNED, THIS IS A SPECIAL CASE OF RISCH, ISN'T IT? 
%% and to hyperexponential functions by~\cite{bostan13a}. 
...
%MK: DELETED THE FOLLOWING THREE LINES BECAUSE THEY ARE ALREADY GLOBALLY DEFINED IN THE PREVIOUS SECTION.
...
For the sake of self-containedness, we give the full proof of this result here.

\begin{lemma}\label{lemma:pole_at_inf}
Let $A$ be an algebraic extension of~$K(x)$ and let
$B=\{\omega_1,\dots,\omega_n\}$ be an integral basis for $A$ that is normal at
...
$\omega_i$ has no poles at finite places and therefore no zero at infinity).
\item The basis element $\omega_i$ is not constant and $f_i\neq0$. This
also implies that $(f_i+g_i)\omega_i$ has a pole at infinity.
\end{enumerate}
...
\begin{theorem}\label{thm:intiff0}
  ...
  $f$ has a double root at infinity (i.e., every Puiseux series expansion
  of $f$ at infinity contains only monomials $(1/x)^\alpha$ with $\alpha\geq2$). Let $B=\{\omega_1,\dots,\omega_n\}$
...
Chevalley's result~\cite[page 9, Corollary 3 ]{Chevalley1951}.

It is clear that $H$ has no finite poles because $h$ has at most ``simple'' poles
(i.e., all Puiseux expansions of $h$ have only exponents $\geq-1$).
...
$g+H$ must have a pole at infinity; recall that the Hermite reduction produces
$g=\sum_i g_i\omega_i$ with proper rational functions~$g_i$. This contradicts
the assumption that $f=g'+h=(g+H)'$ has at least a double root at infinity,
because the latter implies that $g+H$ has at least a single root at infinity.
\end{proof}
...
that $f$ has neither a pole nor a branch point at~$a$. By the transformation
...
\begin{example}
TODO: {\red it will be nice to have an example here!...}
\end{example}
