...
\[r = \frac{1}{e}UW,\]
where $U = (u_1, \ldots, u_n) \in K[x]^n$ and $e$ is defined as before.
If $\deg_x(M) \leq  \deg_x(e)-1$, then we can decompose $U$ into $U = \phi_W(U_1) + U_2$, where $U_1, U_2\in K[x]^n$
and $U_2\in N_W$. Then the polynomial reduction of~$r$ with respect to~$W$ is
...
\[r = (\tilde U_1 V)' + \frac{1}{a} \tilde U_2 V. \]
This means $r$ is decomposed into the sum of an integrable function
and a non-integrable one that lies in a finite dimensional vector space 
over~$K$.
