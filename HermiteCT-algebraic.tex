...
considered in this paper is not as large as it may seem at first glace, because
we require that the operators under considerations should be fuchsian at all
...
$\tilde{U}_2\in N_{\phi_V}$. This means $r$ is decomposed into the sum of an integrable function
...
Let $W$ be an integral basis of~$A$ that is normal at~$\infty$. Let $T := \diag(x^{-r_1}, \ldots, x^{-r_n}) \in K[x]^{n\times n}$
