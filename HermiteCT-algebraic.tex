...
\begin{example}
We would like to compute a minimal telescoper for the function
\[f := -x^{-2}\log\bigl(x^{-2}-t^2\bigr)\sqrt{(x+t)/(x-t)}.\]
...
infinity. When differentiating the $\nu_i$, we obtain the matrix~$B$ and the
denominator $a=x^2(t^2x^2-1)$. Writing the Hermite remainder in terms of the
$\nu_i$, we see that all denominators divide~$a$. Hence $d=1$ and
in~\eqref{EQ:h} we get $u_i=h_i$ and $v_i=0$. By investigating the eigenvalues
of $\lc(B)$ we find that $\ell=1$ and the echelon form of $M_\ell$ is
\[
  \begin{pmatrix}
  ...
  \end{pmatrix}.
\]
The Hermite remainder of~$f$ after polynomial reduction is $-x^2/a\nu_2$.
Next, we consider the derivative $\partial_tf=tx^3\partial_x+2tx^2+x\in A$
whose Hermite remainder is
\[
