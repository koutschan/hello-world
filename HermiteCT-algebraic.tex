...
If $q\in \im(\phi_V)$, then $q = \phi_V(p) = B \cdot p$ for some~$p \in K^n$, which implies
...
for any~$i=1, \ldots, n$. We now consider the space $K[x]_{\lambda_0+\mu}^n$
and its basis~$\cS_{\lambda_0+\mu}$. In the basis of~$\cS_{\lambda_0+\mu}$, we can
express all of the columns of the matrix $J_{\lambda_0}$
as vectors in~$K^{n(\lambda_0+\mu+1)}$. Let~$C({J_{\lambda_0}})$ be the subspace of $K[x]_{\lambda_0+\mu}^n$ generated by
these columns over~$K$. By the same argument as in {\it Case 1}, we have
\[K[x]_{\lambda_0+\mu}^n = C({J_{\lambda_0}}) \oplus N_{\phi_V}. \]
So $\dim_K(N_{\phi_V}) = n(\lambda_0+\mu+1) - \dim(C({J_{\lambda_0}}))$, and
a basis of $N_{\phi_V}$ can be computed by
looking at the echelon form of the matrix $C({J_{\lambda_0}})$.
As an easy consequence of the above discussions about the complement space $N_{\phi_V}$, we get the following result.
...
It is also clear that such a basis will be integral at every other point $a\in\bar C$, because the
multipliers $x^{-r_i}$ are locally units at~$a$.
...
Set $T = \diag(x^{-r_1}, \ldots, x^{-r_n})$, we then have~$V = T\cdot W$.
