...
$h=\sum_{i=1}^n c_i\omega_i$. From
\begin{equation}\label{eq:T}
  \bigl((x-a)^{\mu-1} \partial_x (x-a)^{1-\mu}\bigr) \cdot T =
  (1-\mu+\alpha)(x-a)^{\alpha-1}\log(x-1)^\beta + \beta(x-a)^{\alpha-1}\log(x-a)^{\beta-1}
\end{equation}
it follows that at least one term with $(x-a)^{\alpha-1}$ appears in
$\bigl(v^{\mu-1}\partial_xv^{1-\mu}\bigr)\cdot (h\cdot y_i)$; here we use the
assumption that $\mu>1$, because for $\mu=1$ and $\alpha=\beta=0$ the
right-hand side of~\eqref{eq:T} is zero. This calculation reveals that
