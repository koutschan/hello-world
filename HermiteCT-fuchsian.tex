...
enables us to find the minimal telescoper for $f$ (first approach); 2) the finite-dimensional property
of the $K$-vector space generated by $\bigl\{\,[\partial_t^if]:i\in\set N\,\bigr\}$ leads to
the existence of telescopers and also to a bound on their orders (second approach).
...
followed by polynomial reduction) has the same finite-dimensionality property for the
space of remainders, but without using the double-root assumption on the inputs.
...
Then
\[\sum_{i=1}^s \Bigl(n-\dim\bigl(\text{Im}(H-\lambda_iI_n)\bigr)\Bigr)\leq n,\]
