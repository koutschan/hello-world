...
\begin{lemma}\label{lemma:bound-exps}
  ...
  at $n$ distinct exponents in~$\set N$ (i.e., $N$ counts the
...
Our version below is slightly different from these, and also from the reduction given in~\cite{..} for
...
In the case of algebraic functions~\cite{..}, it is a consequence of Chevalley's theorem that the exponents
...
$\gcd(a, b_{1, 1}, b_{1, 2}, \ldots, b_{n ,n})=1$. By Lemma~\ref{..}, we may assume that
...
\begin{defi}
Let the map $\phi_V\colon K[x,x^{-1}]^n \rightarrow K[x,x^{-1}]^n$
be defined by $\phi_V(P) = aP' + PB$ for any $P\in K[x,x^{-1}]^n$.
...
Then the module $K[x,x^{-1}]^n$ viewed as a $K$-vector space is generated by
\[
...
Therefore, by the combination of Hermite reduction and polynomial
reduction, we get the following theorem.
