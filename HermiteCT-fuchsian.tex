...
\author{\medskip
Shaoshi Chen$^{1,2}$, \, Mark van Hoeij$^{3}$, \, Manuel Kauers$^{4}$, \, Christoph Koutschan$^{5}$ \\
\smallskip
       ...
       \smallskip
       \affaddr{$^3$Department of Mathematics,  Florida State University, FL 32306-4510, (USA)}\\
       \smallskip
       \affaddr{$^4$Institute for Algebra, Johannes Kepler University, Altenberger Stra\ss e 69,
 A-4040 Linz, (Austria)}\\
        \smallskip
       \affaddr{$^5$RICAM, Austrian Academy of Sciences, Altenberger Stra\ss e 69, A-4040 Linz, (Austria)}\\
       \smallskip
...
with a squarefree denominator. In this case, we can take $[f]=f-g'$.
...
In this paper, we show that Trager's Hermite reduction for algebraic functions
directly gives rise to a reduction-based creative telescoping algorithm via
the first approach (Section~\ref{sec:canonic}). We will also generalize his
Hermite reduction to fuchsian D-finite functions in Section~\ref{sec:hermite}
and then combine it with a second reduction, called polynomial reduction
(Section~\ref{sec:polynomial}), which together give rise to a reduction-based
creative telescoping algorithm for fuchsian D-finite functions via the second
approach.  This gives a new bound for the order of the telescopers,
...
\]
for all $f\in A$. This relation is the counterpart of the Bezout relation in the algebraic case.
...
at~$0$, while $\log(x)$ does not. Note also that the terminology only refers
to $x$ but not to~$t$.
...
%\todo{the following remark was added for algebraic functions. 
%Say something about fuchsian functions that are integral everywhere.}
%Because of Chevalley's theorem~\cite[page 9, Corollary 3]{Chevalley1951}, any
%non-constant algebraic function has at least one pole. Equivalently, an
%element~$f$ is integral at all $a\in\bar C\cup\{\infty\}$ if and only if it is
%constant.
...
According to the following lemma, the sum is bounded from below, so the procedure must terminate after a finite
number of iterations.

\begin{lemma}
  Let $\{\omega_1,\dots,\omega_n\}$, $\{\nu_1,\dots,\nu_n\}$, $m_{i,j}$ and $r_1,\dots,r_n$ be as above.
  Let $N$ the number of points $a\in\bar C\cup\{\infty\}$ where at least one of the $\omega_i$ has at least
  one exponent that is not in~$\set N$. Then
  \[
    r_1+\cdots+r_n \geq -\tfrac12 n(n-1)(N-1).
  \]
\end{lemma}
\begin{proof}
  We show that when $r_i\in\set Z$ is such that $x^{-r_i}\omega_i$ is locally integral at infinity,
  then $r_i\geq -\frac12 (n-1)(N-1)$, for every~$i$. 
  ...
  \]
  When all exponents of the series associated to $\omega_i$ at $a\in\bar C$ belong to~$\set N$, then
  ...
  Next, if $r\in\set Z$ is such that $x^{-r}\omega_i$ is integral at infinity, then we must have
  $\defect_\infty(x^{-r}\omega_i)\geq -\frac12n(n-1)$, i.e., 
  \[
    rn + \defect_\infty(\omega_i) \geq -\tfrac12n(n-1),
  \]
  i.e.,
  \begin{alignat*}1
    r &\geq \frac1n\Bigl( -\frac12n(n-1) - \defect_\infty(\omega_i) \Bigr) \\
      &\geq -\tfrac12 (n-1) - \tfrac12 (n-1)(N-2) = -\tfrac12 (n-1)(N-1),
  \end{alignat*}
  as claimed. 
\end{proof}
...
\begin{lemma}
  ...
  and let $e_1,\dots,e_n\in\set Z$ be such that $\{x^{e_1}\omega_1,\dots,x^{e_n}\omega_n\}$ is
  a local integral basis at infinity.
  Then $\{\,x^j\omega_i : i=1,\dots,n; j=0,\dots,e_i\,\}$ is a $K$-vector space basis of~$V$.
\end{lemma}
\begin{proof}
  It is clear that $V$ is closed under taking $K$-linear combinations, so it is clearly a vector space.
  We show that $\{\,x^j\omega_i : i=1,\dots,n; j=0,\dots,e_i\,\}$ is a basis.

  Every $x^j\omega_i\in B$ is by definition integral at every finite place and because of $0\leq j\leq e_i$ also
  ...
  If we had $\deg(p_i)>e_i$ for some~$i$, then $p_i\omega_i$ would not be integral at infinity
  (by definition of the numbers~$e_i$), and then, because $\{\omega_1,\dots,\omega_n\}$ is normal
  ...
  It follows that $\deg(p_i)\leq e_i$ for all~$i$, and therefore $f$ is a $K$-linear combination
  of elements of~$B$.
\end{proof}
...
\begin{equation}\label{eq:modv}
  ...
  \sum_{i=1}^n uv^\mu g_i\left(v^{1-\mu}\omega_i\right)' \mod v.
\end{equation}
By Lemma~\ref{lemma:ibv} and from $\gcd(u,v)=1$ it follows that
the elements $uv^\mu(v^{1-\mu}\omega_i)'$ form a local integral basis
...
\begin{align*}
  \sum_{i=1}^n f_i\omega_i
  &= \sum_{i=1}^n ug_i \bigl( v\omega_i' - (\mu-1)v'\omega_i \bigr) \mod v\\
...
It was observed that Hermite reduction itself often takes less time than the
construction of an integral basis. If Hermite reduction is applied to some
...
Hermite reduction.
\todo{expand this paragraph and adapt it to the fuchsian case.}
...
property~\cite[page 50, Theorem 1]{trager84}. As this result was never
published elsewhere, and for the sake of completeness, we reproduce his proof
here.

\todo{Lemma~\ref{lemma:pole_at_inf} and Theorem~\ref{thm:intiff0} are due to Trager
and refer to the algebraic case. If we want to keep them as they are, this fact has
to made clear in the notation. Or we generalize them to the fuchsian case.}
...
\begin{enumerate}
\item The polynomial~$f_i$ has positive degree.  This means that $f_i+g_i$ has
  a pole at infinity, because the $g_i$ are proper rational functions.  Thus
  $(f_i+g_i)\omega_i$ has a pole at infinity, because $\omega_i$ has no poles
  at finite places and therefore no root at infinity.
  \todo{this last statement is not true in the fuchsian case.}
\item The integral basis element $\omega_i$ is not constant and $f_i$ is not
  zero. Hence $\omega_i$ has a pole at infinity, and this also implies that
  $(f_i+g_i)\omega_i$ has a pole at infinity, again employing the fact that
  $g_i$ is a proper rational function.
\end{enumerate}
...
\begin{theorem}\label{thm:intiff0}
  ...
  $(1/x)^\alpha$ with $\alpha\geq2$).  Let $W=\{\omega_1,\dots,\omega_n\}$ be
  an integral basis for $A$ that is normal at infinity.  If $f=g'+h$ is the
  result of the Hermite reduction with respect to~$W$, then $h=0$ if and only
  if $f$ is integrable in~$A$.
\end{theorem}
...
\todo{at the moment, we don't have this theorem in the fuchsian case.}
