...
\begin{prop}\label{PROP:order}
  ...
  Then $f$ admits a telescoper of order at most $n\deg_x(E)+\sum_{i=1}^n\max(0,\tau_i-1)$.
\end{prop}
\begin{proof}
  ...
  $g_i'$. By Lemma~\ref{lemma:degM} the quantities $m_{j,i}/e$ are proper
  rational functions (TODO: this lemma uses a local integral basis at
  infinity, but the $\omega_i$ are only normal at infinity; somehow the
  $\tau_i$ should enter here). Hence the exponents~$\alpha$ in the expansion of
  $g_i'+\sum_{j=1}^n g_j(m_{j,i}/e)$ satisfy $\alpha\geq\dots$ (TODO: adapt
  the rest with the new estimate on the exponents of $g'$).

  It now follows that the expansion of any $h_i/E$ at infinity cannot have any
  exponents~$\alpha$ smaller than $\min(1,2-\tau_i)$, because such terms could
  not be canceled by $f_i/D$ or $g_i$.

  We have thus shown that $\deg_x(h_i)\leq\deg_x(E)-\min(1,2-\tau_i)$.  In
  ...
  for $i=1,\dots,n$ and $j=0,\dots,\deg_x(E)-\min(1,2-\tau_i)$.  The dimension
  of this space is $n\deg_x(E)+\sum_{i=1}^n\max(0,\tau_i-1)$.  As the Hermite
  remainders of $\partial_t\cdot f,\partial_t^2\cdot f,\dots$ also belong to
  this $K$-vector space, the claimed bound on the order of the telescoper
  follows.
\end{proof}
...
proof of Proposition~\ref{PROP:finite} is generically empty.
\end{remark}
