...
For an element $f\in A$, let $n_f$ be the minimal order of an operator $L\in R[\partial_x]\setminus\{0\}$ with $L\cdot f=0$.
Then the defect of $f\in A$ at $a\in\bar C\cup\{\infty\}$, denoted $\defect_a(f)$, is defined 
as the sum of the exponents of $f$ at $a$ minus
$\sum_{k=0}^{n-1}k=\frac12n_f(n_f-1)$, then the Fuchs relation~\citep{schlesinger95,ince26} says that we have
\[
...
rational functions.  This reduction was later extended to elementary
