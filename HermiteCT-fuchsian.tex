...
$\tau_i$ cannot be negative. In the more general fuchsian situation, this is no longer the case. 
...
we obtain $\sum_{i=1}^n\frac{x^{\lambda-\tau_i}s_i}{x^\lambda e}\omega_i$, where now
...
\]
where the $\tilde s_i$ also belong to a finite dimensional subspace of~$x^{-\tau}K[x]$.
Let us write $K[x]_{\eta,\mu}$ for the subspace of $K[x,x^{-1}]$ consisting of all Laurent polynomials
whose terms are at least $\eta$ and at most~$\mu$. Then $x^{-\tau}K[x]=K[x]_{-\tau,\infty}$, and
...
\begin{defi}
  Let 
  \[
    \phi_V\colon K[x]_{-\tau,\infty}^n \to K[x]_{-\tau,\infty}^n,\quad
    \phi_V(P) = x^\lambda eP' + PB.
  \]
  MK: STILL NOT OK; $K[x]_{-\tau,\infty}$ IS NOT CLOSED UNDER DIFFERENTIATION;
  $(x^{-\tau})'=-\tau x^{-\tau-1}$.
    
  We call $\phi_V$ the \emph{map for polynomial reduction} with respect to~$V$, and call
  the subspace
\[
 \im(\phi_V) = \{\phi_V(P) \mid P \in K[x]_{-\tau,\infty}^n\}
\]
...
\]
When $\eta,\mu\in\set Z$ both are finite, then any element $P\in K[x]_{\eta,\mu}^n$ can be expressed
in the basis $\cS_{\eta,\mu}$ as a vector $\vec{P}\in K^{n(\mu-\eta+1)}$.
...
For any $P\in K[x]_{-\tau,\infty}^n$, there exist $P_1\in K[x]_{-\tau,\infty}^n$ and~$P_2\in N_V$ such that
...
\smallskip
{\it Case 1.}
Assume that $\deg_x(B) < \delta$. For any $P\in K[x]_{-\tau,\infty}^n$ of degree~$\mu$, we have
\[
...
Assume that $\deg_x(B)=\delta$. For any $P\in K[x]_{-\tau,\infty}^n$ of degree~$\mu$, we have
\[
  \lt\bigl(\phi_V(P)\bigr) = \lc(P)(s\lc(e)I_n + \lc(B))x^{\mu+\delta}.
\]
...
Then for any $\mu>\ell$, the matrix $\mu\lc(a)I_n + \lc(B)\in K^{n\times n}$ is invertible.
